...
def rois_overlay_object(data_strf_object):
    preprocess = utilities.min_max_norm(np.std(data_strf_object.images, axis = 0), 0, 255)
...
def contouring_demo(arr_3d, level = None, display_time = True, returns = False, **kwargs):        
    ...
    arr_3d_collapsed = space.collapse_3d(arr_3d, zscore=False)
    # Extract content from masks
    neg_masked, pos_masked = space.rf_mask3d(arr_3d, level = level)
    neg_mask = neg_masked.mask[0]
    pos_mask = pos_masked.mask[1]
    # Extract time courses
    neg_time, pos_time = temporal.extract_timecourse(arr_3d, level = level)
    # Make contours
    if level == None:
        lower_contour, upper_contour = contouring.contour(arr_3d_collapsed)
    else:
        lower_contour, upper_contour = contouring.contour(arr_3d_collapsed, level = (-level, level))
    # Init figure 
...
def stack_to_rgb(stack, eight_bit = True):
    ...
    Notes
    -----
    This function assumes that `utilities.min_max_norm` is a valid function
