...
class Data_strf(Data):
    ...
    def pval_time(self):
        try:
            ...
            self._pval_time = [signal_analysis.bootstrap_time(x, bootstrap_n=500) for x in self.strfs]
            return self._pval_time
    ...
    def pval_space(self):
        try:
            ...
            self._pval_space = [signal_analysis.bootstrap_space(x, bootstrap_n=250) for x in self.strfs]
            return self._pval_space

    @property
    def contours(self, pval_thresh_time = 0.05, pval_thresh_space = 0.05):
        try:
...
#            return self._contours_area
    ...
    @property 
    def strf_masks(self, level = None, pval_thresh_time = 0.05, pval_thresh_space = 0.05):
        try:
            ...
            if self.strfs is np.nan:
                ...
                pval_fail_time = np.argwhere(np.array(self.pval_time) > pval_thresh_time)
                pval_fail_space = np.argwhere(np.array(self.pval_space) > pval_thresh_space)
    ...
    def spectral_tuning_functions(self):
        if self.multicolour == True:
...
#            mins = np.min(self.collapse_times().data, axis = (1, 2))
            ...
            return np.flip(tuning_functions.T, axis = 1) #transpose for simplicity, invert for UV - R by wavelength (increasing)
        else:
...
def load_from_hdf5(path):
    ...
    Note that you don't get any of the fancy processing attributes with this
    """
    new_dict = {}
    with h5py.File(path) as HDF5_file:
        metadata = metadata_dict(HDF5_file)
        for key in HDF5_file.keys():
            new_dict[key] = np.array(HDF5_file[key])
    data_dict = new_dict
...
def load_strf_data(filename, img_stack = True, strfs = True, ipl_depths = True, keys = True, fix_oversize = False):
    with h5py.File(filename) as HDF5_file:
    ...
    Dat_strf_obj = Data_strf(strfs = strfs_arr, ipl_depths = ipl_depths, 
        images = images, rois = rois, metadata = meta_data, strf_keys = keys, multicolour = multicolour_bool)
...
def compile_strf_df(files, summary_prints = True):
    roi_stat_list = []
    rec_info_list = []
    for i in files:
...
def compile_chroma_strf_df(files, summary_prints = True):
    roi_stat_list = []
    rec_info_list = []
    for i in files:
...
def load_strf_by_df_index(df, index):
    roi = df["roi"][index]
    path = df["path"][index]
