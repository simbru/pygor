...
import utilities
import filehandling
from filehandling import get_rel_objective_XYZ, get_experiment_datetime, metadata_dict
import utilities
...
class Data:
    ...
    def __post_init__(self):
        ...
        self.rois = h5data.ROIs
        self.images = h5data.wDataCh0_detrended
        try:
            self.ipl_depths = h5data.Positions
        except AttributeError:
            warnings.warn(f"HDF5 key 'Positions' not found for file {filename}", stacklevel=2)
            self.ipl_depths = np.nan
        # if 'FFF' in self.data_types:
        #     print("ding")
            # raise AttributeError("Missing 'position' wave from IGOR")

@dataclass
class Data_FFF(Data):
    # Data.__post_init__(self.filename)
    # Intrinsic attrs
    #filename   : str or path
    phase_num  : int = np.nan
    ...
    def __post_init__(self):
        ...
        if isinstance(pathlib.Path(self.filename), pathlib.Path) is True:
            self.filename = pathlib.Path(self.filename)
        self.name = self.filename.stem
        self.averages = h5data.Averages0
    ...
    def __post_init__(self):
        ...
        with h5py.File(self.filename) as HDF5_file:
            ...
            if multicolour_bool is True:
                ...
                self.numcolour = 1
            self.strfs = filehandling.load_strf(HDF5_file)
