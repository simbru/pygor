...
#         org_arr = utilities.auto_remove_border(org_arr)
#     # Collapse space
...
def bootstrap_time(arr_3d, bootstrap_n = 2500, mode_param = 2, mode = "sd", 
    ...
    if np.ma.is_masked(arr_3d) == True:
        with warnings.catch_warnings():
            ...
            arr_3d = utilities.auto_remove_border(arr_3d)
...
def bootstrap_space(arr_3d, bootstrap_n = 250, collapse_time = np.ma.std, metric = np.max, plot = False, parallel = True, seed = None,**kwargs): # these metrics work so leave them
    """
    Perform a spatial permutation test to compute p-value for a given metric on the spatial data.
