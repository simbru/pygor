...
def get_submodules(main_module_name) -> list[object]:
    ...
    for submodule_name in dir(main_module):
        ...
        if (
            isinstance(submodule, type(main_module))
            and submodule.__package__ == main_module.__package__
        ):
...
def get_submodule_class_names(main_module_name):
    ...
    for submodule_name in dir(main_module):
        ...
        if (
            ...
            classes = [
                cls.__name__
                for cls in vars(submodule).values()
                if isinstance(cls, type) and cls.__module__ == submodule.__name__
            ]
            if len(classes) == 1:
