...
class STRF(Core):
    ...
    def __post_init__(self):
        ...
        if self.filename.suffix == ".h5":
            with h5py.File(self.filename) as HDF5_file:
                ...
                keys = [
                    i
                    for i in HDF5_file.keys()
                    if pattern.match(i) and any(i[4] == np.arange(0, 10).astype("str"))
                ]
                self.strf_keys = natsort.natsorted(keys)
                # Set bool for multi-colour RFs and ensure multicolour attributes set correctly
                strf_colour_int_list = [
                    int(n.removeprefix("STRF").split("_")[-1]) for n in keys
                ]
                if (
                    len(np.unique(strf_colour_int_list)) == 1
                ):  # only one colour according to STRF keys
                    multicolour_bool = False
                else:
                    multicolour_bool = True
                self.strf_dur_ms = (
                    try_fetch_table_params(HDF5_file, "Noise_FilterLength_s") * 1000
                )
    ...
    def _validate_data_consistency(self):
        ...
        if hasattr(self, "ipl_depths") and self.ipl_depths is not None:
            if (
                hasattr(self.ipl_depths, "__len__")
                and len(self.ipl_depths) != self.num_rois
            ):
                import warnings
                ...
                print(
                    f"  -> ipl_depths shape: {getattr(self.ipl_depths, 'shape', 'no shape')}"
                )
        ...
        try:
            ...
            actual_rois_from_strfs = strfs_chroma.shape[
                1
            ]  # shape is [time, roi, y, x] or similar
    ...
    def stim_size(self, upscaling_factor=4):
        ...
        """
        return (
            pygor.utils.unit_conversion.au_to_visang(self.stim_size_arbitrary)
            / upscaling_factor
        )
    ...
    def __calc_pval_time(self, parallel=None, **kwargs: Any) -> np.ndarray:
        ...
        self._pval_time = np.array(
            [
                pygor.strf.bootstrap.bootstrap_time(
                    x, bootstrap_n=self.bs_settings["time_bs_n"], parallel=parallel
                )
                for x in bar
            ]
        )
    ...
    def __calc_pval_space(self, parallel=None, **kwargs: Any) -> np.ndarray:
        ...
        self._pval_space = np.array(
            [
                pygor.strf.bootstrap.bootstrap_space(
                    x, bootstrap_n=self.bs_settings["space_bs_n"], parallel=parallel
                )
                for x in bar
            ]
        )
    ...
    def update_bootstrap_settings(self, update_dict, silence_print=False) -> None:
        ...
        if silence_print == False:
            if not_allowed_keys or not_found_keys:
                ...
                if not_allowed_keys:
                    print(
                        "The following disallowed keys were passed:", *not_allowed_keys
                    )
                if not_found_keys:
                    print(f"Keys not found in self.bs_settings: {not_found_keys}")
                if none_default_key:
                    print(
                        f"Keys set to default values: {[(i, default_dict[i]) for i in none_default_key]}"
                    )
        ...
        if self.bs_settings["do_bootstrap"] == True:
            try:
                ...
                self.__calc_pval_time(
                    parallel=parallel
                )  # Executes calculation and writes to self._pval_time
            return self._pval_time
        ...
        if self.bs_settings["do_bootstrap"] == True:
            try:
                ...
                self.__calc_pval_space(
                    parallel=parallel
                )  # Executes calculation and writes to self._pval_space
            return self._pval_space
    ...
    def get_pvals_table(self) -> pd.DataFrame:
        if self.multicolour == True:
            ...
            final_arr = np.hstack(
                (space_vals, time_vals, both_sig, any_sig), dtype="object"
            )
            column_labels = [
                "space_R",
                "space_G",
    ...
    def strfs_no_border(self) -> np.ndarray:
        if (
            not hasattr(self, "_strfs_no_border_cache")
            or self._strfs_no_border_cache is None
        ):
    ...
    def fit_contours(
        ...
        if self.bs_settings["do_bootstrap"] == True:
            ...
            __contours = [
                pygor.strf.contouring.bipolar_contour(
                    arr
                )  # ensures no contour is drawn if pval not sig enough
                if time_pvals[count] < self.bs_settings["time_sig_thresh"]
                and space_pvals[count] < self.bs_settings["space_sig_thresh"]
    ...
    def get_contours_area(self, scaling_factor=None) -> list:
        ...
        return [
            pygor.strf.contouring.contours_area_bipolar(
                __contours, scaling_factor=scaling_factor
            )
            for __contours in self.fit_contours()
        ]
    ...
    def calc_contours_centroids(self) -> np.ndarray:
        try:
            ...
            off_contours = [
                pygor.strf.contouring.contour_centroid(i)
                for i in self.fit_contours()[:, 0]
            ]
            on_contours = [
                pygor.strf.contouring.contour_centroid(i)
                for i in self.fit_contours()[:, 1]
            ]
            self.__contours_centroids = np.array(
                [off_contours, on_contours], dtype="object"
            )
            return self.__contours_centroids
        ...
        try:
            ...
            self.__centres_by_pol = np.array(
                [
                    [
                        np.average(i, axis=0)
                        for i in self.calc_contours_centroids()[0, :]
                    ],
                    [
                        np.average(i, axis=0)
                        for i in self.calc_contours_centroids()[1, :]
                    ],
                ]
            )
            return self.__centres_by_pol
    ...
    def get_contours_centres(self, center_on="amplitude") -> np.ndarray:
        ...
        if center_on == "largest":
            pos_conts_cents = np.array(
                [
                    i[0] if i.size != 0 else np.array([np.nan, np.nan])
                    for i in self.calc_contours_centroids()[1, :]
                ]
            )
            neg_conts_cents = np.array(
                [
                    i[0] if i.size != 0 else np.array([np.nan, np.nan])
                    for i in self.calc_contours_centroids()[0, :]
                ]
            ...
            neg_pos_largest = np.array(
                [(i[np.argmax(i)], j[np.argmax(j)]) for i, j in area_cents]
            )
            xs = np.where(
                neg_pos_largest[:, 0] > neg_pos_largest[:, 1],
                neg_conts_cents[:, 0],
                pos_conts_cents[:, 0],
            )
            ys = np.where(
            ...
            return np.array(
                [xs, ys]
            ).T  # centres by biggest area, irrespective of polarity
    ...
    def calc_contours_complexities(self) -> np.ndarray:
        return pygor.strf.contouring.complexity_weighted(
            self.fit_contours(), self.get_contours_area()
        )
    ...
    def get_timecourses(
        ...
        if method == "contour":
            ...
            if spoof_masks is True:
                ...
                masked_strfs.mask[empty_mask_indices] = spoofed_masks[
                    empty_mask_indices
                ]
    ...
    def get_pix_times(self, incl_borders=False):
        if incl_borders is True:
            ...
            return np.array(
                [np.reshape(i, (i.shape[0], -1)) for i in self.strfs_no_border]
            )
    ...
    def get_strf_masks(
        self, roi=None, level=None, force_recompute=False
        ...
        def set_strf_masks(roi=roi):
            ...
            all_masks = np.array(
                [pygor.strf.contouring.bipolar_mask(i) for i in colapse_times]
            )
            # print(all_masks.shape)
            all_masks = np.repeat(
                np.expand_dims(all_masks, 2), self.strfs.shape[1], axis=2
            )
            # print(all_masks.shape)
            # Apply mask to expanded and repeated strfs (to get negative and positive)
            strfs_expanded = np.repeat(
                np.expand_dims(self.strfs[roi], axis=1), 2, axis=1
            )
            all_strfs_masked = np.ma.array(
                strfs_expanded, mask=all_masks, keep_mask=True
            )
    ...
    ## Methods
    def calc_LED_offset(
        self, reference_LED_index=[0, 1, 2], compare_LED_index=[3]
    ) -> np.ndarray:
        """
        ...
        if self.multicolour == True:
            ...
            avg_reference_pos = np.nanmean(
                np.take(avg_colour_centre, reference_LED_index, axis=0), axis=0
            )
            avg_compare_pos = np.nanmean(
                np.take(avg_colour_centre, compare_LED_index, axis=0), axis=0
            )
    ...
    def centre_strfs(self):
        ...
        strf_shifted = np.ma.array(
            [np.roll(arr, shift_by[i], axis=(1, 2)) for i, arr in enumerate(self.strfs)]
        )
        return strf_shifted
    ...
    def collapse_times(
        ...
        **kwargs: Any,
        ...
        roi_key = (
            tuple(roi)
            if roi is not None
            and hasattr(roi, "__iter__")
            and not isinstance(roi, (str, int))
            else roi
        )
        cache_key = (
            roi_key,
        ...
        if isinstance(iterate_through, Iterable):
            ...
            target_shape = (1, self.strfs.shape[2], self.strfs.shape[3])
        collapsed_strf_arr = np.ma.empty(target_shape)
        for n, roi in enumerate(iterate_through):
            collapsed_strf_arr[n] = pygor.strf.spatial.collapse_3d(
                strfs_arr[roi], zscore=zscore, **kwargs
            )
        if spatial_centre == True:
            ...
            contours_centers = np.where(
                self.get_contours_centres() > 0,
                np.floor(self.get_contours_centres()),
                np.ceil(self.get_contours_centres()),
            )
            ...
            shifted = np.ma.array(
                [np.roll(arr, shift_by[i], axis=(0, 1)) for i, arr in enumerate(arr3d)]
            )
    ...
    def calc_spatial_correlations(
        self, abs_arrays=True, signal_only=True, single_channel_value=np.nan
        ...
        for roi in range(self.num_rois):  # Process ALL ROIs
            ...
            roi_correlations = np.full(
                len(pair_names), single_channel_value, dtype=float
            )
            ...
            if len(valid_channels) >= 2:
                ...
                for pair_idx, (ch_i, ch_j) in enumerate(
                    zip(pairs_indices[0], pairs_indices[1])
                ):
        ...
        full_df = pd.DataFrame(
            roi_results, columns=pair_names, index=range(self.num_rois)
        )

        return full_df, pair_names

    def spatial_overlap_index_stats(
        self,
        abs_arrays=True,
        signal_only=True,
        force_recompute=False,
        single_channel_value=np.nan,
    ):
        """
        ...
        all_corr_pairs, pair_names = self.calc_spatial_correlations(
            abs_arrays=abs_arrays,
            signal_only=signal_only,
            single_channel_value=single_channel_value,
        )
    ...
    def spatial_overlap_index_mean(
        self,
        abs_arrays=True,
        signal_only=True,
        force_recompute=False,
        ...
        return self.spatial_overlap_index_stats(
            abs_arrays=abs_arrays,
            signal_only=signal_only,
    ...
    def spatial_overlap_channel_pair(
        self, ch1, ch2, abs_arrays=True, signal_only=True, single_channel_value=np.nan
    ):
        ...
        if pair_name not in all_corr_pairs.columns:
            raise ValueError(
                f"Channel pair {pair_name} not found. Available: {list(all_corr_pairs.columns)}"
            )
        return all_corr_pairs[pair_name].to_numpy()

    # Convenience methods for specific pairs
    def spatial_overlap_red_uv(
        self, abs_arrays=True, signal_only=True, single_channel_value=np.nan
    ):
        """Red-UV spatial overlap (Ch0-Ch3) - strongest anti-correlation expected"""
        return self.spatial_overlap_channel_pair(
            0, 3, abs_arrays, signal_only, single_channel_value
        )

    def spatial_overlap_green_blue(
        self, abs_arrays=True, signal_only=True, single_channel_value=np.nan
    ):
        """Green-Blue spatial overlap (Ch1-Ch2)"""
        return self.spatial_overlap_channel_pair(
            1, 2, abs_arrays, signal_only, single_channel_value
        )

    def spatial_overlap_red_green(
        self, abs_arrays=True, signal_only=True, single_channel_value=np.nan
    ):
        """Red-Green spatial overlap (Ch0-Ch1)"""
        return self.spatial_overlap_channel_pair(
            0, 1, abs_arrays, signal_only, single_channel_value
        )

    def spatial_overlap_blue_uv(
        self, abs_arrays=True, signal_only=True, single_channel_value=np.nan
    ):
        """Blue-UV spatial overlap (Ch2-Ch3)"""
        return self.spatial_overlap_channel_pair(
            2, 3, abs_arrays, signal_only, single_channel_value
        )

    def spatial_overlap_green_uv(
        self, abs_arrays=True, signal_only=True, single_channel_value=np.nan
    ):
        """Green-UV spatial overlap (Ch1-Ch3)"""
        return self.spatial_overlap_channel_pair(
            1, 3, abs_arrays, signal_only, single_channel_value
        )

    def spatial_overlap_red_blue(
        self, abs_arrays=True, signal_only=True, single_channel_value=np.nan
    ):
        """Red-Blue spatial overlap (Ch0-Ch2)"""
        return self.spatial_overlap_channel_pair(
            0, 2, abs_arrays, signal_only, single_channel_value
        )
    ...
    def get_polarities(
        ...
        if mode == "spatial":
            ...
            opponent_threshold = 0.8  # Could be made a parameter
            ...
            cat[valid_mask & (np.abs(polarity_indices) <= opponent_threshold)] = (
                2  # Opponent
            )
            return cat
            ...
            return pygor.strf.polarity.get_polarities(
                self, roi, exclude_FirstLast, mode, force_recompute
            )
    ...
    def spatial_polarity_index(
        self,
        roi=None,
        ch_idx=None,
        threshold=3,
        mask_by_channel=True,
        mask_threshold=2,
        dimstr="time",
    ) -> np.ndarray:
        ...
        if ch_idx is not None:
            if not self.multicolour:
                if ch_idx == -1 or ch_idx == 0:
                    ...
                    raise ValueError(
                        f"ch_idx={ch_idx} specified but object is not multicolour (only has 1 channel, use 0 or -1)"
                    )
            else:
    ...
    def get_opponency_bool(self) -> bool:
        if self.multicolour == True:
            ...
            opponent_bool = [
                False if len(np.unique(i[~np.isnan(i)])) == 1 | 0 else True for i in arr
            ]
            return opponent_bool
        else:
            raise AttributeError(
                "Operation cannot be done since object property '.multicolour' is False"
            )
    ...
    def calc_centre_distances(self, mode="cs_seg"):
        if mode == "cs_seg":
            ...
            overall_center = np.nanmean(
                rf_centres, axis=0, keepdims=True
            )  # Shape: (1, 2)

            # Calculate distance of each STRF from overall center
            euclidian_dist = np.sqrt(
                np.sum((rf_centres - overall_center) ** 2, axis=1)
            )  # Shape: (n_total_strfs,)
    ...
    def calc_pca_rf_shape_analysis(
        ...
        if debug:
            print(f"DEBUG: Processing strf_list = {strf_list} for roi = {roi}")
            print(
                f"DEBUG: all_collapsed shape = {all_collapsed.shape}, all_cs_maps shape = {all_cs_maps.shape}"
            )
        ...
        for i, strf_idx in enumerate(strf_list):
            ...
            if center_pixel_count == 0:
                ...
                if debug:
                    ...
                    print(
                        f"DEBUG STRF {strf_idx}: mask_unique_values={unique_mask_vals}, center_pixels={center_pixel_count}"
                    )
                    print(
                        f"DEBUG STRF {strf_idx}: No center pixels - returning NaN and continuing to next STRF"
                    )
                ...
                if debug:
                    print(
                        f"DEBUG STRF {strf_idx}: Added NaN values, about to continue..."
                    )
                continue
            ...
            if np.sum(label_mask) > 0:
                ...
                if np.max(labeled_center) > 0:
                    ...
                    component_sizes = np.bincount(labeled_center.flat)[
                        1:
                    ...
                    if len(component_sizes) > 0:
                        ...
                        if debug:
                            print(
                                f"DEBUG STRF {strf_idx}: Found {len(component_sizes)} connected components, using largest with {component_sizes[largest_component - 1]} pixels"
                            )
            ...
            if debug:
                print(
                    f"DEBUG STRF {strf_idx}: pos_pixels={pos_pixels}, neg_pixels={neg_pixels}"
                )
            ...
            if pos_pixels >= neg_pixels:
                ...
                if debug:
                    print(
                        f"DEBUG STRF {strf_idx}: using positive pixels, significant_count={np.sum(significant_mask)}"
                    )
            else:
                # Use negative pixels
                significant_mask = (strf_data < -threshold_sd) & label_mask
                if debug:
                    print(
                        f"DEBUG STRF {strf_idx}: using negative pixels, significant_count={np.sum(significant_mask)}"
                    )
            ...
            if debug and np.sum(significant_mask) > 0:
                ...
                if len(unique_mask_vals) == 1 and unique_mask_vals[0] != 0:
                    print(
                        f"DEBUG STRF {strf_idx}: CRITICAL ERROR - {np.sum(significant_mask)} pixels selected but mask has no center region!"
                    )
            ...
            eccentricity = (
                np.sqrt(1 - (eigenvalues[1] / eigenvalues[0]))
                if eigenvalues[0] > 0
                else 0
            )
            ...
            if (
                plot
                and roi is not None
                and isinstance(roi, (int, np.integer))
                and strf_idx == roi
            ):
    ...
    def calc_centre_surround_metrics(
        self, roi=None, force_recompute=False, debug=False
    ):
        """
        ...
        for strf_idx in strf_list:
            ...
            region_a_sum = (
                np.sum(strf_data[region_a_map]) / region_a_map.sum()
                if region_a_map.sum() > 0
                else np.nan
            )
            region_b_sum = (
                np.sum(strf_data[region_b_map]) / region_b_map.sum()
                if region_b_map.sum() > 0
                else np.nan
            )
            noise_sum = (
                np.sum(strf_data[noise_map]) / noise_map.sum()
                if noise_map.sum() > 0
                else np.nan
            )
            ...
            if (
                np.isnan(region_a_sum)
                or np.isnan(region_b_sum)
                or (abs(region_a_sum) == 0 and abs(region_b_sum) == 0)
            ):
                ...
                if abs(secondary_response) == 0:
                    ...
                    secondary_percentage = (
                        abs(secondary_response) / abs(dominant_response) * 100
                    )
                ...
                if abs(dominant_response) == 0:
                    ...
                    if opposite_polarity:
                        ...
                        opponency_index = 1 - (
                            abs(dominant_response) - abs(secondary_response)
                        ) / (abs(dominant_response) + abs(secondary_response))
                    else:
    ...
    def get_dominance_ratio(self, roi=None, force_recompute=False):
        ...
        return self.calc_centre_surround_metrics(
            roi=roi, force_recompute=force_recompute
        )["dominance_ratio"]
    ...
    def calc_colour_channel_offsets(
        self,
        roi=None,
        mode="cs_seg",
        label=0,
        ...
        if mode == "cs_seg":
            ...
            channel_centers = self.get_seg_centres_by_channel(
                label=label, weighted=weighted
            )
            ...
            spaces = (
                self.collapse_times_by_channel()
            )  # shape: (n_colours, n_cells, height, width)
                    ...
                    if min_valid or max_valid:
                        ...
                        if min_valid and max_valid:
                            if np.abs(min_val) >= np.abs(max_val):
                                extremum_coords = np.unravel_index(
                                    np.nanargmin(space), space.shape
                                )
                            else:
                                extremum_coords = np.unravel_index(
                                    np.nanargmax(space), space.shape
                                )
                        elif min_valid:
                            extremum_coords = np.unravel_index(
                                np.nanargmin(space), space.shape
            ...
            for colour_idx in range(n_colours):
                for cell_idx in range(n_cells):
                    ...
                    if (
                        not np.isnan(neg_centroid).any()
                        and not np.isnan(pos_centroid).any()
                    ):
        ...
        invalid_centers_mask = np.isnan(channel_centers).any(axis=2) | np.isnan(
            true_centers[np.newaxis, :, :]
        ).any(axis=2)
            ...
            if minimal_plot:
                    ...
                    if not np.isnan([channel_y, channel_x]).any():
                        ...
                        if not np.isnan([true_y, true_x]).any():
                            plt.annotate(
                                "",
                                xy=(true_x, true_y),
                                xytext=(channel_x, channel_y),
                                arrowprops=dict(
                                    arrowstyle="->", lw=2, color=color, alpha=0.9
                                ),
                            )
                ...
                if not np.isnan([true_y, true_x]).any():
                    plt.plot(
                        ...
                        markeredgecolor="black",
                        markeredgewidth=3,
                        label=f"True center: ({true_x:.1f}, {true_y:.1f})",
                    )
                    ...
                    if not np.isnan([channel_y, channel_x]).any():
                        ...
                        if not np.isnan([true_y, true_x]).any():
                            plt.annotate(
                                ...
                                xytext=(channel_x, channel_y),
                                arrowprops=dict(
                                    arrowstyle="->", lw=2, color=color, alpha=0.8
                                ),
                            )
    ...
    def pca_rf_shape_analysis(
        ...
        """
        results = self.calc_pca_rf_shape_analysis(
            roi=roi,
    ...
    def get_time_amps(self, **kwargs: Any) -> np.ndarray:
        ...
        largest_mag = np.where(
            maxes > np.abs(mins), maxes, mins
        )  # search and insert values to retain sign
        return largest_mag
    ...
    def get_opponency_index_time(self, **kwargs: Any) -> np.ndarray:
        ...
        antagonism_index = np.where(
            opposite_polarity,
            1
            - (abs(dominant_amps) - abs(secondary_amps))
            / (abs(dominant_amps) + abs(secondary_amps)),
    ...
    def calc_tunings_amplitude(
        self, dimstr="time", treshhold=2, **kwargs: Any
    ) -> np.ndarray:
        if self.multicolour == True:
            if dimstr == "time":
            ...
            if treshhold != None:
                tuning_functions = np.where(
                    np.abs(tuning_functions) > treshhold, tuning_functions, np.nan
                )
    ...
    def calc_tunings_area(
        self, size=None, upscale_factor=4, largest_only=True
    ) -> np.ndarray:
        if self.multicolour == True:
            ...
            if size == None:
                ...
                neg_contour_areas = [
                    i[0]
                    for i in self.get_contours_area(
                        unit_conversion.au_to_visang(size) / upscale_factor
                    )
                ]
                pos_contour_areas = [
                    i[1]
            ...
            if largest_only == True:
                ...
                max_negs = np.max(
                    pygor.utilities.numpy_fillna(neg_contour_areas).astype(float),
                    axis=1,
                )
                max_pos = np.max(
                    pygor.utilities.numpy_fillna(pos_contour_areas).astype(float),
                    axis=1,
                )
            ...
            raise AttributeError(
                "Operation cannot be done since object contains no property '.multicolour."
            )
    ...
    def calc_spectral_centroids(self) -> tuple[np.ndarray, np.ndarray]:
        spectroids_neg = np.apply_along_axis(
            pygor.strf.temporal.only_centroid, 1, self.get_timecourses()[:, 0]
        )
        spectroids_pos = np.apply_along_axis(
            pygor.strf.temporal.only_centroid, 1, self.get_timecourses()[:, 1]
        )
    ...
    def get_biphasic_index(self) -> np.ndarray:
        vals = np.apply_along_axis(
            pygor.strf.temporal.biphasic_index, 1, self.get_timecourses()[:, 0]
        )
        pass_bool = self.check_cs_pass()
        # nan where pass bools is False
        return np.where(pass_bool, vals, np.nan)
    ...
    def get_strf_peak_times(
        self, roi=None, mask=None, interpolate=True, upscale_factor=10
    ):
        """
        ...
        if mask is not None:
            if isinstance(mask, (int, float)):
                ...
                mask_3d = (
                    np.abs(self.collapse_times()) > mask
                )  # (n_rois, height, width)
                ...
                if mask.ndim == 2:
                    ...
                    if roi is not None:
                        ...
                        raise ValueError(
                            "2D mask provided but no ROI specified. Use 3D mask or specify roi."
                        )
                elif mask.ndim == 3:
                    mask_3d = mask.astype(bool) if mask.dtype != bool else mask
                else:
                    raise ValueError(
                        "Mask must be 2D (for single ROI) or 3D (for all ROIs)"
                    )
            else:
        ...
        if interpolate:
            ...
            strf_flat = (
                strf_data.reshape(n_rois, n_frames, -1)
                .transpose(1, 0, 2)
                .reshape(n_frames, -1)
            )
            ...
            strf_upscaled = strf_upscaled_flat.reshape(
                len(t_upscaled), n_rois, height, width
            ).transpose(1, 0, 2, 3)
        ...
        if mask_3d is not None:
            ...
            mask_expanded = np.repeat(
                ~mask_3d[:, np.newaxis, :, :], data_to_analyze.shape[1], axis=1
            )
            strf_masked = np.ma.masked_array(data_to_analyze, mask=mask_expanded)
            strf_max_indices = np.ma.argmax(
                np.ma.abs(strf_masked), axis=1
            )  # (n_rois, height, width)
            strf_max_times = np.ma.masked_array(
                time_array[strf_max_indices], mask=~mask_3d
            )
        else:
            strf_max_indices = np.argmax(
                np.abs(data_to_analyze), axis=1
            )  # (n_rois, height, width)
    ...
    def get_strf_delta_times(
        self, roi=None, channel=None, use_segmentation=False, seg_kwargs=None
    ):
        ...
        if use_segmentation:
            ...
            full_masks = (
                np.sum(seg_masks, axis=(1, 2))
                == seg_masks.shape[1] * seg_masks.shape[2]
            )
    ...
    def demo_rf_coherence(self, roi, use_segmentation=True, amplitude_weighted=False):
        ...
        if amplitude_weighted:
            ...
            if amp_sum == 0:
                warnings.warn(
                    f"ROI {roi}: amplitude sum is zero, falling back to uniform weights."
                )
                n_valid = mask.sum()
        ...
        if w_valid.size > 0 and w_valid.max() > 0:
            ...
            vmin, vmax = (
                float(np.nanmin(np.asarray(lmap))),
                float(np.nanmax(np.asarray(lmap))),
            )
        # Ensure some range
        if vmin == vmax:
            vmin -= 0.001
        ...
        axes[1, 2].set_title(
            f"6. Coherence = {coherence:.2f}, Dir = {direction:.0f}\u00b0"
        )
    ...
    def get_latency_reliable(self, use_segmentation=True, coherence_threshold=0):
        ...
        results = self.compute_latency_vectors(
            use_segmentation=use_segmentation, coherence_threshold=coherence_threshold
        )
    ...
    def compute_temporal_span(
        self, roi=None, use_segmentation=True, force_recompute=False
    ):
        ...
        for i in range(n_rois):
            ...
            if hasattr(lmap, "mask") and lmap.mask.any():
                valid = (
                    ~lmap.mask & np.isfinite(lmap_arr) & np.isfinite(amp) & (amp > 0)
                )
            else:
    ...
    def get_temporal_span(self, use_segmentation=True):
        ...
        np.ndarray of shape (n_rois,)
        """
        return self.compute_temporal_span(use_segmentation=use_segmentation)[
            "weighted_sd"
        ]
    ...
    def get_temporal_span_raw(self, use_segmentation=True):
        ...
        np.ndarray of shape (n_rois,)
        """
        return self.compute_temporal_span(use_segmentation=use_segmentation)["raw_span"]

    # -- Latency plane fit --

    def compute_latency_plane_fit(
        self, roi=None, use_segmentation=True, force_recompute=False
    ):
    ...
    def get_plane_fit_direction(self, use_segmentation=True):
        ...
        np.ndarray of shape (n_rois,)
        """
        return self.compute_latency_plane_fit(use_segmentation=use_segmentation)[
            "direction"
        ]
    ...
    def plot_latency_vectors(
        ...
        for i, r in enumerate(rois):
            ...
            compressed = (
                lmap.compressed() if hasattr(lmap, "compressed") else lmap.ravel()
            )
            ...
            if not use_segmentation:
                ...
                amp_threshold = (
                    np.percentile(finite_amp, 95) if finite_amp.size > 0 else 0
                )
                ...
                _cbar_mappable = cur_ax.imshow(
                    lmap, origin="lower", cmap="jet", vmin=vmin, vmax=vmax
                )
            ...
            if reliable and mag > 0:
                ...
                cur_ax.set_title(
                    f"dir: {results['direction'][i]:.1f}\N{DEGREE SIGN}, coh: {coh:.2f}, mag: {mag:.4f}",
                    fontsize=10,
                )
            else:
    ...
    def calc_spectrums(self, roibyroi=False) -> tuple[np.ndarray, np.ndarray]:
        spectrum_neg = np.array(
            [pygor.strf.temporal.only_spectrum(i) for i in self.get_timecourses()[:, 0]]
        )
        spectrum_pos = np.array(
            [pygor.strf.temporal.only_spectrum(i) for i in self.get_timecourses()[:, 1]]
        )
    ...
    def demo_contouring(self, roi, chromatic_reshape=False):
        ...
        neg_c, pos_c = pygor.strf.contouring.bipolar_contour(
            self.collapse_times(roi), plot_results=True, ax=ax
        )
        ...
        pygor.strf.contouring.bipolar_contour(
            self.collapse_times(roi), result_plot=True, ax=ax[:, 1:5]
        )
    ...
    def plot_chromatic_overview(
        self, roi=None, contours=False, with_times=False, colour_idx=None, **kwargs: Any
    ):
        """
        ...
        **kwargs
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            return pygor.strf.plotting.advanced.chroma_overview(
                self,
                roi,
                contours=contours,
    ...
    def play_strf(self, roi, dur_s=None, **kwargs: Any):
        ...
        if isinstance(roi, tuple):
            ...
            anim = pygor.plotting.play_movie(
                np.squeeze(self.strfs[roi]), dur_s=dur_s, **kwargs
            )
        return anim
    ...
    def play_strf_rgb(
        self, roi, channel="All", method="grey_centered", plot=True, **kwargs: Any
    ):
        ## Generate RGB movie
        if channel == "All":
            ...
            arr = np.array(
                [
                    np.moveaxis(self.get_chroma_strf(roi)[i], [0, 1], [-1, 0])
                    for i in channel
                ]
            )
            # arr = np.hstack(arr)
        ...
        if len(channel.shape) == 1:
            arr = np.expand_dims(
                np.array(
                    np.moveaxis(self.get_chroma_strf(roi)[channel], [0, 1], [-1, 0])
                ),
                0,
            )
        else:
    ...
    def cs_seg(self, roi=None, plot=False, force_recompute=False, **kwargs: Any):
        if plot is False:
            if force_recompute is True:
                ...
                return np.squeeze(self._cs_seg_results_maps[roi]), np.squeeze(
                    self._cs_seg_results_times[roi]
                )
            try:
                return np.squeeze(self._cs_seg_results_maps[roi]), np.squeeze(
                    self._cs_seg_results_times[roi]
    ...
    def get_centre_only_seg(
        ...
        if min_area > 1:
            ...
            result = ndimage.binary_erosion(result, structure=structure).astype(
                np.uint8
            )
            result = ndimage.binary_dilation(result, structure=structure).astype(
                np.uint8
            )
        ...
        if use_largest_component:
            ...
            for roi_idx in range(result.shape[0]):
                ...
                if np.max(labeled_components) > 0:
                    # Find the largest connected component
                    component_sizes = np.bincount(labeled_components.flat)[
                        1:
                    ]  # Exclude background (0)
                    if len(component_sizes) > 0:
                        largest_component = np.argmax(component_sizes) + 1
                        ...
                        result[roi_idx] = (
                            labeled_components == largest_component
                        ).astype(np.uint8)
    ...
    def get_seg_areas(self, border_value=1, min_area=5, include_border=False):
        ...
        sums = np.where(
            sums == (self.strfs_no_border.shape[-1] * self.strfs_no_border.shape[-2]),
            np.nan,
            sums,
        )
        ...
        for n, segmap in enumerate(cmaps):
            ...
            if len(centre_coords) == 0 or len(np.unique(segmap)) == 1:
                ...
                if weighted is True:
                    weights = (
                        np.abs(
                            np.squeeze(self.collapse_times(n))[
    ...
    def unravel_strf_indices(self, roi_index=None, colour_index=None, flat_index=None):
        ...
        if hasattr(self, "strf") and self.strf is not None:
            ...
            raise AttributeError(
                "Cannot determine STRF dimensions. Object may not be properly initialized."
            )
        ...
        if flat_index is not None:
            if flat_index >= total_strfs or flat_index < 0:
                raise ValueError(
                    f"flat_index {flat_index} out of range [0, {total_strfs - 1}]"
                )
        ...
        if roi_index is not None and colour_index is not None:
            if roi_index >= n_rois or roi_index < 0:
                raise ValueError(
                    f"roi_index {roi_index} out of range [0, {n_rois - 1}]"
                )
            if colour_index >= n_colours or colour_index < 0:
                raise ValueError(
                    f"colour_index {colour_index} out of range [0, {n_colours - 1}]"
                )
        ...
        raise ValueError(
            "Provide either flat_index, or both roi_index and colour_index"
        )

    def calc_centre_surr_vectors(
        self,
        roi=None,
        mode="weighted",
        angle_range_360=True,
        ...
        if mode == "cs_seg":
            ...
            surrounds = self.get_seg_centres(
                label=1, weighted=True
            )  # shape: (n_rois, 2)
            ...
            raise ValueError(
                f"Unknown mode '{mode}'. Available: 'cs_seg', 'minmax', 'weighted'"
            )
        ...
        if plot:
            ...
            if not np.isnan(plot_centers[original_plot_roi]).any():
                ...
                if mode == "weighted":
                    center_coords_text = (
                        f"OFF (center): ({center_x:.1f}, {center_y:.1f})"
                    )
                else:
                ...
                plt.plot(
                    ...
                    markeredgecolor="black",
                    markeredgewidth=2,
                    label=center_coords_text,
                )

            if not np.isnan(plot_surrounds[original_plot_roi]).any():
                surround_y, surround_x = plot_surrounds[original_plot_roi]
                if mode == "weighted":
                    surround_coords_text = (
                        f"ON (surround): ({surround_x:.1f}, {surround_y:.1f})"
                    )
                else:
                    surround_coords_text = (
                        f"Surround: ({surround_x:.1f}, {surround_y:.1f})"
                    )
                plt.plot(
                    surround_x,
                    surround_y,
                    ...
                    markeredgecolor="white",
                    markeredgewidth=2,
                    label=surround_coords_text,
                )

            # Draw vector if both points are valid
            if (
                not np.isnan(plot_centers[original_plot_roi]).any()
                and not np.isnan(plot_surrounds[original_plot_roi]).any()
            ):
                center_y, center_x = plot_centers[original_plot_roi]
                surround_y, surround_x = plot_surrounds[original_plot_roi]
                ...
                # Add vector info
                mag = (
                    magnitudes[array_index]
                    if hasattr(magnitudes, "__len__")
                    else magnitudes
                )
                angle = (
                    angles_deg[array_index]
                    if hasattr(angles_deg, "__len__")
                    else angles_deg
                )
                plt.text(
                    0.02,
                    0.98,
                    f"ROI {original_plot_roi}\nMagnitude: {mag:.2f}\nAngle: {angle:.1f}deg\nMode: {mode}",
                    transform=plt.gca().transAxes,
                    verticalalignment="top",
    ...
    def get_centre_surr_angles(self, roi=None, mask_singlepol=False):
        ...
        if mask_singlepol is True:
            ...
            vectors["angle_degrees"] = np.where(
                mask_subset, vectors["angle_degrees"], np.nan
            )
    ...
    def convolve_with_img(
        ...
        **kwargs: Any,
        ...
        return convolve.convolve_image(
            self,
            roi,
            img,
            plot=plot,
            auto_crop=auto_crop,
            auto_crop_thresh=auto_crop_thresh,
            xrange=xrange,
            yrange=yrange,
            **kwargs,
        )

    def map_extrema_timing(
        self,
        roi=None,
        threshold=3.0,
        exclude_firstlast=(1, 1),
        return_milliseconds=False,
        frame_rate_hz=15.625,
    ):
        """
    ...
    def analyze_spatial_alignment(
        self, roi, threshold=3.0, reference_channel=0, collapse_method="peak"
    ):
        """
    ...
    def compute_colour_channel_overlap(
        self, roi, colour_channels=(0, 1), threshold=3.0, collapse_method="peak"
    ):
        """
    ...
    def compute_spatial_offset_between_channels(
        self,
        roi,
        colour_channels=(0, 1),
        threshold=3.0,
    ...
    def calculate_strf(
        ...
        **kwargs
        ...
        if normalize_strfs and "strfs" in results:
            ...
            results["strfs_unnormalized"] = results[
                "strfs"
            ].copy()  # Keep original for comparison
            ...
            if verbose:
                print(
                    "Normalization applied. Original STRFs stored in 'strfs_unnormalized' key."
                )
        ...
        if verbose:
            print(
                f"Updated internal attributes: self.strfs shape={self.strfs.shape}, num_strfs={self.num_strfs}"
            )

        return results

    def get_npercent_times(
        self,
        roi=None,
        percent=10,
        incl_borders=False,
        ...
        if use_percentile:
            ...
            if bidirectional:
                ...
                top_indices = np.concatenate(
                    [high_indices[high_sort], low_indices[low_sort]]
                )
        ...
        pix_times = self.get_pix_times(incl_borders=incl_borders)[
            roi
        ]  # Shape: (n_timepoints, n_pixels)
        selected_timecourses = pix_times[
            :, top_indices
        ]  # Shape: (n_timepoints, n_selected_pixels)
    ...
    def get_spatiotemporal_correlation(self, roi=None, method="pearson"):
        ...
        return correlation_module.calculate_spatiotemporal_correlation(
            self, roi=roi, method=method
        )
...
def calculate_spatiotemporal_correlation(strf_obj, roi=None, method="pearson"):
    ...
    return correlation_module.calculate_spatiotemporal_correlation(
        strf_obj, roi=roi, method=method
    )
            ...
            if not self.multicolour:
                ...
                if hasattr(ch_idx, "__iter__") and not isinstance(ch_idx, str):
                    ...
                    for idx in ch_idx:
                        if idx != -1 and idx != 0:
                            raise ValueError(
                                f"ch_idx contains {idx} but object is not multicolour (only has 1 channel, use 0 or -1)"
                            )
                    return reshaped[
                        0
                    ]  # Always return the single channel for any valid indices
                else:
                    # Handle single index
...
for attr_name in dir(STRF):
    ...
    if (
        ...
        and not attr_name.endswith("_by_channel")
