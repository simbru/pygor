...
def auto_remove_border(array):
    ...
    """
    warnings.warn(
        "Cropping border is a destructive method. You lose data, with no way of recovering original shape! If this warning comes from a plotting script, you can likley ignore it."
    )
    ...
    if was_2d == True:
        return np.copy(
            np.squeeze(
                array[
                    :,
                    upper_border_width:-lower_border_wdith,
                    right_border_width:-left_border_width,
                ],
                axis=0,
            )
        )
