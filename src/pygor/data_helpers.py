...
def load_wDataCh0(HDF5_file):
    ...
    if "wDataCh0_detrended" in HDF5_file.keys():
        ...
        warnings.warn(
            "wDataCh0 or wDataCh0_detrended could not be identified. Returning None"
        )
        img = None
...
def fix_oversize_sta(strf_arr4d, boxsize_um, upscale_multiple=4):
    ...
    boxes_tup = (
        np.ceil(pygor.utils.unit_conversion.calculate_boxes_on_screen(size)).astype(
            "int"
        )
        * upscale_multiple
    )
    # Create the appropriate mask
    mask = pygor.utilities.manual_border_mask(
        strf_arr4d[0][0].shape, boxes_tup
    )  # just take shape from first ROI first frame
    ...
    borders_widths = pygor.utilities.check_border(
        new_masked_strfs, expect_symmetry=False
    )
    ...
    new_shape = org_shape - (
        0,
        0,
        borders_widths[0] + borders_widths[1],
        borders_widths[2] + borders_widths[3],
    )
...
def load_strf(HDF5_file, post_process=True, fix_oversize=True):
    ...
    if not strf_list:  # if its empty
        warnings.warn(
            f"HDF5 key 'STRF0_' not found for file {HDF5_file.filename}", stacklevel=2
        )
        return None
    ...
    if input_shape[2] > input_shape[3]:
        warnings.warn(
            f"Rotation detected and corrected for {HDF5_file.filename}", stacklevel=2
        )
    ...
    if fix_oversize == True and experiment_date < datetime.date(
        2023, 4, 4
    ):  # came before < cutoff == True
        warnings.warn(
            "Old experiment detected, correcting for oversized STA", stacklevel=2
        )
        try:
            meta_data = metadata_dict(HDF5_file)
            size = int(
                label_from_str(
                    pathlib.Path(meta_data["filename"]).name,
            ...
            Warning(
                "Size not identified from filename, causing size to equals nan. May require manual solution."
            )
            size = np.nan
...
def label_from_str(
    ...
    def _final_check(input_str, search_terms, **kwargs):
        if hasattr(search_terms, "__iter__") is True:
            ...
            if terms_found:
                if (
                    "first_return_only" in kwargs
                    and kwargs["first_return_only"] == True
                ):
