...
def convolve_image(strf_obj, roi_index, img = "example", img_zoom = 1/2, arr_zoom = 3, plot = False, norm_output = True, auto_crop = True, auto_crop_thresh = 3, xrange = None, yrange = None):
    if img is None:
        img = plt.imread(example_img_path)
    if img == "example":
        img = load_example()
    ...
    arr_list = []
    if strf_obj.multicolour is True:
        loopthrough = np.arange(strf_obj.strfs.shape[0]).reshape(-1, 4).astype(int)[roi_index]
    else:
        loopthrough = np.arange(strf_obj.num_rois)[roi_index]
    if not isinstance(loopthrough, Iterable):
        loopthrough = [loopthrough]
    if auto_crop is True:
        inp_arr = np.abs(strf_obj.collapse_times(loopthrough))
        if strf_obj.multicolour is True:
