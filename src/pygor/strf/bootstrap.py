...
def bootstrap_time(
    ...
    **kwargs,
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        flat_indices = np.argpartition(np.abs(collapse_flat_compressed), -n_top_pix)[
            -n_top_pix:
        ]
    ...
    def _permute_iteration(org_time, rng):
        permuted_time = stationary_shuffle(
            org_time, max_sample_length=len(org_time), rng=rng
        )
        fft_perm = np.abs(np.fft.rfft(permuted_time))[1:]
        perm_stat = metric(fft_perm)
        return perm_stat
    ...
    if not parallel:
        perm_stat_list = np.array(
            [_permute_iteration(org_time, rng) for _ in range(bootstrap_n)]
        )
    if parallel:
        ...
        perm_stat_batches = Parallel(n_jobs=-1)(
            delayed(parallel_permute)(batch) for batch in seed_batches
        )
        perm_stat_list = np.concatenate(perm_stat_batches)

    epsilon = 1e-10
    p_value = (np.sum(perm_stat_list >= org_stat) + epsilon) / (
        bootstrap_n + 1 + epsilon
    )
    if plot == True:
        permuted_time = stationary_shuffle(org_time)
        perm_fft = np.abs(np.fft.rfft(permuted_time))[1:]
        ...
        max_index = np.unravel_index(
            np.argmax(np.max(np.abs(arr_3d), axis=0)), arr_3d[0].shape
        )
        ax[1].plot(arr_3d[:, max_index[0], max_index[1]])
        ax[1].set_title("Temporal filter")
        ax[2].plot(org_time, label="Avg. XY along Z")
        ...
        ax[2].legend()
        ax[3].plot(
            np.fft.rfftfreq(org_time.size, 1 / 15.625)[1:],
            fft_org,
            label="Joined timecourses",
        )  #
        ax[3].plot(
            np.fft.rfftfreq(org_time.size, 1 / 15.625)[1:],
            perm_fft,
        ...
        ax[4].axvline(
            ...
            c="black",
            ls="--",
            label=f"Percentile {np.round(p_value, 5)} at value {np.round(org_stat, 3)}",
        )
        ax[4].legend()
    return p_value
...
def bootstrap_space(
    ...
    **kwargs
    ...
    def _single_permute_compute(inp_arr, rng, array_return=False):
        # Get space stat
        permuted_arr = rng.permuted(
            rng.permuted(inp_arr, axis=1), axis=2
        )  # permute space, leave time alone
    ...
    def _single_resample_compute_new(
        ...
        if x_parts is None and y_parts is None:
            indices = rng.choice(
                inp_arr.size,
                size=(inp_arr.shape[0], inp_arr.shape[1], inp_arr.shape[2]),
                replace=True,
                shuffle=False,
            )
            ...
            def resample_blocks(arr, rng, x_parts, y_parts):
                ...
                y_resampled_blocks = rng.choice(
                    y_splits, size=len(y_splits), replace=True, axis=0
                )
                y_resampled = np.concatenate(y_resampled_blocks, axis=1)
                # Split along the x-axis
                x_splits = np.array_split(y_resampled, x_parts, axis=2)
                x_resampled_blocks = rng.choice(
                    x_splits, size=len(x_splits), replace=True, axis=0
                )
            ...
            resampled_slices = [
                resample_blocks(slice, rng, x_parts, y_parts) for slice in t_splits
            ]
    ...
    epsilon = 1e-10
    p_value = (np.sum(permuted_stat_list >= org_stat) + 1 + epsilon) / (
        bootstrap_n + 1 + epsilon
    )
    ...
    if plot == True:
        ...
        ax[0].set_title("Input data (collapsed)")
        permuted_plot = ax[1].imshow(
            collapse_time(
                function_choice(
                    org_arr, array_return=True, rng=np.random.default_rng()
                ),
                axis=0,
            ),
            origin="lower",
        ...
        ax[1].set_title("Example permutation (collapsed)")
        if "binsize" not in kwargs:
