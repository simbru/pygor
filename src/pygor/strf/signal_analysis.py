...
def stimphase_stat(
    trace, trigger_times, phase_trignum, linespeed, stat=np.std, plot=False, **kwargs
):
    """
    ...
    if inter_phase_dur == 0:
        ...
        for n, i in enumerate(trial_split):
            ...
            if "percentdiff_threshold" in kwargs:
                ...
                if (
                    stat == np.std and repeats > 1
                ):  # stupid solution but cannot be asked
                    counter_stats[n] = np.array(counter_stats[n]) * np.sqrt(repeats)
                if np.sum(np.abs(stats[n])) > np.sum(np.abs(counter_stats[n])):
                    percentage_difference_arr = np.abs(
                        1 - np.divide(counter_stats, stats)
                    )
                else:
                    percentage_difference_arr = np.abs(
                        1 - np.divide(stats, counter_stats)
                    )
    ...
    if plot == True:
        ...
        secax.set_xlim(np.array([-0.75, 5.9]))
        secax.grid(False)
    return stats


def stimphase_stat_diff(
    trace,
    trigger_times,
    phase_trignum,
    linespeed,
...
def stimphase_stat_multi(
    ...
    **kwargs : dict
    ...
    return np.apply_along_axis(
        function, axis, traces_, trigger_times, phase_trignum, linespeed, stat, **kwargs
    ).T
...
def init_timing_vars(file_path):
    ...
    avg_trig_times_ms = np.round(
        avg_trig_times / linespeed, 0
    )  # round so we get an integer index
...
def stim_phase_df(
    file_path_list, conditions_label_list, phase_trignum, repeats, stat=np.std, **kwargs
):
    """
    ...
    for file in file_path_list:
        ...
        dict["colour"] = pygor.data_helpers.label_from_str(
            loaded_file.filename, ["R", "G", "B", "UV"]
        )
        ...
        if "std_threshold" in kwargs:
            ...
            if (
                any(sub_threshold_indeces) is True
            ):  # any as in if any is below threshold
                if (
                    len(sub_threshold_indeces) == traces.shape[1]
                ):  # if all under threshold
        ...
        # Run the test
        result = stimphase_stat_multi(
            traces,
            avg_trig_times_ms,
            phase_trignum,
            linespeed,
            pause_dur=inter_phase_durMs,
            stat=stat,
            avgd_over=repeats,
            function=stimphase_stat,
            **kwargs,
        )
        result_difference = stimphase_stat_multi(
            traces,
            avg_trig_times_ms,
            phase_trignum,
            linespeed,
...
def category_labels(
    ...
    **kwargs,
    ...
    if minmax_scale == True:
        ax_abs_per_row = np.max(
            np.abs(spectral_tuning_functions), axis=1, keepdims=True
        )
        spectral_tuning_functions_scaled = spectral_tuning_functions / ax_abs_per_row
        spectral_tuning_functions_scaled_thresh = np.where(
            np.abs(spectral_tuning_functions_scaled) > thresh,
            spectral_tuning_functions_scaled,
            0,
        )
        ...
        all_u, all_b, all_g, all_r = [
            i
            for i in np.where(
                np.abs(spectral_tuning_functions.T) > thresh,
                spectral_tuning_functions.T,
                0,
            )
        ]
    # Generate label bool arrays
    _short_bias = (
        (np.abs(all_u) > np.abs(all_b))
        & (np.abs(all_r) < np.abs(all_u))
        & (
            (np.abs(all_b) > np.abs(all_g))
            | (np.abs(all_u) > np.abs(all_b) + np.abs(all_g) + np.abs(all_r))
        )
    )
    # _long_bias = (np.abs(all_u) < np.abs(all_b)) & (np.abs(all_b) < np.abs(all_g)) & (np.abs(all_g) < np.abs(all_r))
    _long_bias = (
        (np.abs(all_b) < np.abs(all_g))
        & (np.abs(all_g) < np.abs(all_r))
        & (np.abs(all_u) < np.abs(all_r))
        & (np.abs(all_u) < np.abs(all_g))
    ) | (np.abs(all_r) > (np.abs(all_g) + np.abs(all_b) + np.abs(all_u)))
    _mid_bias = (
        (np.abs(all_b) + np.abs(all_g) > (np.abs(all_u) + np.abs(all_r)))
        & ((np.abs(all_r) < np.abs(all_g)) & (np.abs(all_b) > np.abs(all_u)))
        & ((np.sign(all_b) == np.sign(all_g)) & (np.sign(all_u) == np.sign(all_r)))
    )
    _b_bias = (
        (np.abs(all_b) > np.abs(all_u))
        & (np.abs(all_b) > np.abs(all_g))
        & (np.abs(all_g) + np.abs(all_r) < np.abs(all_b))
    )
    _g_bias = (
        (np.abs(all_g) > np.abs(all_b))
        & (np.abs(all_g) > np.abs(all_r))
        & (np.abs(all_u) + np.abs(all_b) < np.abs(all_g))
    )
    # Broad: apply if all polarities are the same and nothing else applies
    _broad = (
        np.abs(all_r > 0) & np.abs(all_g > 0) & np.abs(all_b > 0) & np.abs(all_u > 0)
    )
    ...
    _short_opp = (((all_r + all_g > 0) & (all_u < 0)) & (all_b > 0)) | (
        ((all_r + all_g < 0) & (all_u > 0)) & (all_b < 0)
    )
    _long_opp = ((all_r > 0) & (all_g < 0)) | (
        (all_r < 0) & (all_g > 0)
    )  # (R>0 && G<0) || (R<0 && G>0)
    _mid_opp = ((all_g > 0) & (all_b < 0)) | (
        (all_g < 0) & (all_b > 0)
    )  # (R>0 && B<0) || R<0 && B>0)

    # Give priority to opponency versus the other catagories
    _short_bias = (
        _short_bias * (_short_bias * (_short_bias != _short_opp))
        & (_short_bias * (_short_bias != _mid_opp))
        & (_short_bias * (_short_bias != _long_opp))
    )
    _mid_bias = (
        _mid_bias * (_mid_bias * (_mid_bias != _short_opp))
        & (_mid_bias * (_mid_bias != _mid_opp))
        & (_mid_bias * (_mid_bias != _long_opp))
    )
    _long_bias = (
        _long_bias * (_long_bias * (_long_bias != _short_opp))
        & (_long_bias * (_long_bias != _mid_opp))
        & (_long_bias * (_long_bias != _long_opp))
    )
    _g_bias = (
        _g_bias * (_g_bias * (_g_bias != _short_opp))
        & (_g_bias * (_g_bias != _mid_opp))
        & (_g_bias * (_g_bias != _long_opp))
    )
    _b_bias = (
        _b_bias * (_b_bias * (_b_bias != _short_opp))
        & (_b_bias * (_b_bias != _mid_opp))
        & (_b_bias * (_b_bias != _long_opp))
    )
    _broad = (
        _broad * (_broad * (_broad != _short_opp))
        & (_broad * (_broad != _mid_opp))
        & (_broad * (_broad != _long_opp))
    )
    # # Mid also yields versus the other groups
    _mid_bias = _mid_bias * (
        (_mid_bias != _b_bias)
        & (_mid_bias != _g_bias)
        & (_mid_bias != _long_bias)
        & (_mid_bias != _short_bias)
    )
    ...
    if "print_reminder" in kwargs and kwargs["print_reminder"] == True:
        print(
            "Label arrays in order: Long-biased, short-biased, mid-biased, v-biased, broad/achromatic, short-opponent, mid-opponent, long-opponent"
        )
    if "print_var_names" in kwargs and kwargs["print_var_names"] == True:
        print(
            "Vars: long_bias, short_bias, mid_bias, v_bias, broad, short_opp, mid_opp, long_opp"
        )
    ...
    if plot == True:
        ...
        fig, axs = plt.subplots(
            1, 10, figsize=(10 * 3, 1 * 3), sharex=False, sharey=True
        )
        ...
        for n, i in enumerate(titles):
            axs[n].set_title(i)
        plt.savefig(
            r"C:\Users\SimenLab\OneDrive\Universitet\PhD\Conferences\ERM 2023\Chromaticity graphs\tuning_functions.svg"
        )
...
def on_off_metric(
    on_segment,
    off_segment,
    on_crop_first_last=(0, 0),
    off_crop_first_last=(0, 0),
    metric="auc",
):
    on_segment = on_segment[
        on_crop_first_last[0] : len(on_segment) - on_crop_first_last[1]
    ]
    off_segment = off_segment[
        off_crop_first_last[0] : len(off_segment) - off_crop_first_last[1]
    ]
    if metric == "auc":
...
def pop_split_by_colour(pop_array, stim_mode):
    ...
    if len(pop_array) % 2 != 0:
        ...
        pop_array = pop_array[
            : len(pop_array) - (math.ceil(len(pop_array) / 4 % 2 * stim_mode))
        ]
    split_pop = np.array(
        np.hsplit(
            pop_array.T,
            np.arange(
                int(len(pop_array) / stim_mode),
                len(pop_array),
                int(len(pop_array) / stim_mode),
            ),
        )
...
def on_off_split(response_segment, on_len, off_len):
    ...
    try:
        return np.array(
            np.split(response_segment, [int(on_off_proportion * segment_length)])
        )
    except ValueError:
        return np.array(
            np.split(response_segment, [int(on_off_proportion * segment_length)]),
...
def pop_on_off_colour_split(pop_array, stim_mode, on_len, off_len, axis=2):
    ...
    try:
        return np.array(
            np.split(colour_pop, [int(segment_length * on_off_proportion)], axis=axis)
        )
    except ValueError:
        return np.array(
            np.split(colour_pop, [int(segment_length * on_off_proportion)], axis=axis),
...
def pop_on_off_colour_index(
    ...
    if on_crop_first_last != (0, 0) or off_crop_first_last != (0, 0):
        ...
        prepped_arr.mask[
            0, :, :, on_crop_first_last[0] : length - on_crop_first_last[1]
        ] = 0
        prepped_arr.mask[
            1, :, :, off_crop_first_last[0] : length - off_crop_first_last[1]
        ] = 0
...
def pop_estimate_peaktime(pop_array, smoothing_window=1):
    ...
    pop_processed = np.apply_along_axis(
        np.convolve, 0, pop_array, np.ones(smoothing_window)
    )
