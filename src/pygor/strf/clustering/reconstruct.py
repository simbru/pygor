...
def reconstruct_cluster_spatial(clust_df, cluster_id_str, parallel=True):
    ...
    target_arr = clust_df.query(f"cluster_id == '{cluster_id_str}'")[
        ["roi", "strf_obj"]
    ].to_numpy()  # makes it possible to unpack

    # Define a loop
    def _loop(obj, roi):
        _unpack_me = pygor.utilities.multicolour_reshape(
            obj.collapse_times(spatial_centre=True), 4
        )[:, roi]
        return _unpack_me
    ...
    # Otherwise we use joblib
        ...
        collate_strfs = Parallel(n_jobs=-1)(
            delayed(_loop)(obj, roi) for roi, obj in target_arr
        )
    else:
        raise AttributeError(
            "'parallel' must be either True, False, None or a joblib.parllel.Parallel object"
        )
...
def reconstruct_cluster_temporal(clust_df, cluster_id_str, parallel=True):
    ...
    # Define a loop
    def _loop(obj, roi):
        _unpack_me = pygor.utilities.multicolour_reshape(obj.get_timecourses(), 4)[
            :, roi
        ]
        return _unpack_me
...
def reconstruct_cluster_strf(clust_df, cluster_id_str):
    ...
    for roi, obj in clust_df.query(f"cluster_id == '{cluster_id_str}'")[
        ["roi", "strf_obj"]
    ].to_numpy():
        ...
        uv_strf.append(
            uv
        )  # The line `r_strf = np.average(r_strf, axis = 0)` is calculating the
