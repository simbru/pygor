...
from pygor.strf.clustering.clustering_funcs import (
    cols_like,
    df_GMM,
    apply_clusters,
    prep_input_df,
...
def run_clustering(clust_df):
    ...
    output_df = pd.DataFrame(
        preprocessor.fit_transform(pruned_df.copy()),
        columns=cols_like(clustering_params, pruned_df),
    )
    # output_df = pruned_df[]
    ...
    split_df = pygor.strf.analyse.split_df_by(
        output_df.join(
            pruned_df.filter(like="pol"), how="inner", validate="one_to_one"
        ),
        "cat_pol",
    )
    ## Step 2: Apply PCA to each split independently
    pca_results = {
        key: df_pca(split_df[key].filter(regex=clust_params_regex), whiten=False)
    ...
    clust_dict = {
        i: clust_pipeline(pca_df_dict[i].filter(like="PC"))
        for i in split_df.keys()
        if i != "empty"
    }
    ## Ste 3.5: Rename cluster IDs to prep for merging
    for i in clust_dict.keys():
        if clust_dict[i]["cluster_id"].dtype != "object":
            split_df[i]["cluster_id"] = (
                f"{i}" + "_" + clust_dict[i]["cluster_id"].astype(str)
            )
            clust_dict[i]["cluster_id"] = (
                f"{i}" + "_" + clust_dict[i]["cluster_id"].astype(str)
            )
    ...
    merged_stats_df = merged_stats_df[
        merged_stats_df.columns.drop(list(merged_stats_df.filter(like="pol_")))
    ]
    ...
    org_stats_df = prep_input_df(
        org_stats_df, scaler=None, select_cols=clustering_params
    ).reset_index()
