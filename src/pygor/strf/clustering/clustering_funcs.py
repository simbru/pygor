...
def display_parallel_coordinates(df, num_clusters):
    ...
    for i in range(num_clusters):
        ...
        pc = parallel_coordinates(
            cluster_points[i], "cluster", color=[addAlpha(palette[i], 0.5)]
        )
...
def remove_nondata(
    input_df,
    to_drop=["date", "path", "filename", "curr_path", "strf_keys", "cell_id", "size"],
):
...
def remove_nonnumeric(
    input_df, numerics=["int16", "int32", "int64", "float16", "float32", "float64"]
):
    input_df = input_df.copy()
...
def remove_missing_vals(
    input_df,
    ignore_cols=["ipl_depths"],
    numerical_only=True,
    return_numerical_only=False,
):
    # Prevent overwriting
    input_df = input_df.copy()
    # Numerical or not
    ...
    if ignore_cols != None:
        purge_df = purge_df_numerical.drop(
            [i for i in ignore_cols if i in input_df.columns], axis=1
    ...
    if return_numerical_only == True:
        ...
        for ignored_col, i in zip(
            ignore_cols, [purge_df_numerical.columns.get_loc(i) for i in ["ipl_depths"]]
        ):
...
def prep_input_df(
    ...
    if scaler != None:
        ...
        if ignore_scale != None:
            ...
            ignored_indeces = [
                input_df.columns.get_loc(c) for c in ignore_scale if c in input_df
            ]
        ...
        if ignore_scale != None:  # add ignored cols back in their original order:)
            for column_index, column_name in zip(ignored_indeces, ignore_scale):
                input_df.insert(
                    column_index, column_name, temp_extract_cols[column_name].values
                )  # = temp_extract_cols
    return input_df
...
def df_kmeans(
    input_df, n_clusters=15, random_state=0, n_init="auto", apply_cluster_id=True
):
    input_df = input_df.copy()
...
def df_GMM(
    ...
    if n_components == "auto":
        ...
        max_index_vals = score[
            np.where(score[:, 2].astype("float") == np.min(score[:, 2].astype("float")))
        ][0]
        ...
        print(
            "Automatic number determined as AIC =",
            smallest_bic,
            "landing on covariance_type =",
            covariance_type,
            "with ",
            n_components,
            "n_components",
...
def df_AggHierarchy(
    ...
    ag_model = AgglomerativeClustering(
        n_clusters=None, distance_threshold=dist_thresh, compute_distances=True
    ).fit(input_df)
...
def df_pca(input_df, n_comps="auto", whiten=False):
    ...
    return pca, pd.DataFrame(
        ...
        columns=[f"PC{i}" for i in range(len(pca.explained_variance_))],
    )
