...
def play_movie(d3_arr, dur_s=1.3, figaxim_return=False, rgb_repr=False, **kwargs):
    ...
    if rgb_repr is True:
        if d3_arr.shape[-1] != 3:
            raise AttributeError(
                "Expected RGB representation in final index. Shape should be: (time,y,x,RGB)"
            )
...
def play_movie_4d_simple(
    ...
    if d4_arr.ndim != 4:
        raise AttributeError(
            "Array passed to function is not four dimensional (4D). Please use 'play_movie' instead."
        )
    # Return default matplotlib plotting parameters to new dict and change those needed
    plot_settings = plt.rcParams
    plot_settings["animation.html"] = "jshtml"
    ...
    if d4_arr.ndim != 4:
        raise AttributeError(
            "Array passed to function is not four dimensional (4D). Please consider 'play_movie' instead, or check inputs."
        )
    ...
    with matplotlib.rc_context(rc=plot_settings):
        ...
        animation = matplotlib.animation.FuncAnimation(
            fig,
            video,
            interval=dur_s / frames_time * 1000,
            frames=frames_time,
            repeat_delay=500,
        )
        plt.close()
...
def play_movie_4d(
    ...
    **kwargs,
    ...
    # Reshape and treat sequentially (for sanity)
        ...
        raise AttributeError(
            "Input dimensions are ambigious or unpexted. Revise input arrays."
        )
    ...
    with matplotlib.rc_context(rc=plot_settings):
        ...
        fig, axs = plt.subplots(
            ...
            frameon=frameon,
            gridspec_kw={"wspace": 0.0, "hspace": 0.0, "bottom": 0.01, "top": 0.99},
        )
        for n, ax in enumerate(axs.flat):
            fig = plt.gcf()
            # Hide grid lines
        ...
        # Create the animation based on the above function
        animation = matplotlib.animation.FuncAnimation(
            fig, video, frames=frames, interval=dur_s / frames * 1000, repeat_delay=500
        )
        plt.close()
        return animation
