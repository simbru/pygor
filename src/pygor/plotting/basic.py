...
def _legacy_play_movie(d3_arr, **kwargs):
    ...
    def update_plots(frame):
        ...
        if "cmap" in kwargs:
            ...
            STRF_map = ax.imshow(
                img_arr[frame], cmap="Greys_r", **kwargs, origin="lower"
            )
    ...
    slider = widgets.IntSlider(
        min=0, max=img_len, step=1, value=1, continuous_update=True
    )
...
def contouring_demo(arr_3d, level=None, display_time=True, returns=False, **kwargs):
    ...
    if level == None:
        ...
        lower_contour, upper_contour = pygor.strf.contouring.contour(
            arr_3d_collapsed, level=(-level, level)
        )
    # Init figure
    if display_time == True:
        fig, ax = plt.subplots(
            1,
            3,
            subplot_kw={"aspect": "auto", "adjustable": "datalim"},
            figsize=(20, 5),
        )
    ...
    cax1 = divider1.append_axes(
        "right", size="2%", pad=0.05
    )  # autoscaling "pretend" axis to append colorbar to
    ...
    ## 2: Plot masks
    mask = ax[1].imshow(
        np.invert(neg_mask * -1) + np.invert(pos_mask), origin="lower", cmap="RdBu"
    )
    if returns == True:
        return fig, ax, im
...
def basic_stim_overlay(
    ...
    xy_loc=(3, 3),
    size=10,
    colour_list=custom.fish_palette,
):
    """
    ...
    for i in range(repeat_interval):
        ...
        ] = np.array(colors.to_rgb(colour_list[i])) * 255
    return rgb_array
    ...
    if plot_type == "traces":
        ...
        if num_traces < 10:
            fig, axs = plt.subplots(
                num_traces, figsize=(4, num_traces), sharex=True, sharey=True
            )
        if num_traces > 10:
            fig, axs = plt.subplots(
                num_traces, figsize=(4, int(num_traces / 2)), sharex=True, sharey=True
            )
        for n, ax in enumerate(axs.flatten()):
            ...
            if mode != None:
                ...
                if on_dur == None and off_dur == None:
                    ...
                    for reps in range(mode * 2)[::2]:  # a bit madness but works
                        ax.axvspan(
                            assumed_onoff_dur * reps,
                            assumed_onoff_dur * reps + assumed_onoff_dur,
                            color="lightgrey",
                            lw=0,
                            alpha=0.25,
                        )
