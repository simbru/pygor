...
def _get_user_config_path() -> Path:
    """Get path to user config file (~/.pygor/config.yaml)."""
    return Path.home() / ".pygor" / "config.yaml"


def _get_project_config_path() -> Path:
    """Get path to project config file (./pygor.yaml)."""
    return Path.cwd() / "pygor.yaml"


def _load_yaml_file(path: Path) -> dict:
    """Load a YAML file, returning empty dict if not found or yaml not installed."""
    if not HAS_YAML:
        return {}
...
def load_config() -> dict:
    ...
    # Merge user config
    user_config = _load_yaml_file(_get_user_config_path())
    config = _deep_merge(config, user_config)
    
    # Merge project config (highest priority)
    project_config = _load_yaml_file(_get_project_config_path())
...
def create_user_config_template():
    """
    Create a template config file at ~/.pygor/config.yaml.
