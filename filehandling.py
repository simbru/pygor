...
#            return self._contours_area
    ...
    def polarity_category(self):
        ...
        for i in arr:
            ...
            if not any(inner_no_nan):
                ...
                result.append("on")
            elif -1&1 in inner_no_nan and 2 not in inner_no_nan:
                result.append("opp")
    ...
    def save_pkl(self, save_path, filename):
        save_pkl(self, save_path, filename)
...
def picklestore_objects(input_objects, output_folder, do_bootstrap = True):
    if isinstance(input_objects, Iterable) is False:
        input_objects = [input_objects]
    output_folder = pathlib.Path(output_folder)
    # progress_bar = alive_it(input_objects, spinner = "fishes", bar = 'blocks', calibrate = 50, force_tty=True)
    progress_bar = tqdm(input_objects, desc = "Iterating through, loading, and storing listed files as objects")
